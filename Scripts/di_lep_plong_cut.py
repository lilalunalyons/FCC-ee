...
for entries in rootfile.electron:
	...
	if iev == iev_prev:
		...
		if num_of_electrons >= 2:
			...
			# calculate longitudinal momentum for these 2 leptons and add together
			x = electron_pt[i_max]*np.sinh(electron_eta[i_max]) + electron_pt[i_second_max]*np.sinh(electron_eta[i_second_max])
			if x > -50.0:
				lepton_Pls.append(x)
		iev_prev = iev
		num_of_electrons = 1
...
for entries in rootfile.muon:
	...
	if iev == iev_prev:
		...
		if num_of_muons >= 2:
			...
			# calculate longitudinal momentum from these 2 leptons
			x = (muon_pt[i_max]*np.sinh(muon_eta[i_max])) + (muon_pt[i_second_max]*np.sinh(muon_eta[i_second_max]))
			if x > -50.0:
				lepton_Pls.append(x)
		iev_prev = iev
		num_of_muons = 1
...
lepton_Pl = ROOT.TH1D(key1, "Di-lepton Longitudinal Momentum;Longitudinal Momentum (GeV/c);Number of Events/ GeV", 100, -45, 45)
for x in lepton_Pls:
    lepton_Pl.Fill(x)

canvas.cd(1)
lepton_Pl.Draw()
canvas.SaveAs("pl_dilep_cut.pdf")
